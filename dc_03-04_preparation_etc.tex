...
\newcommand{\現在までの研究状況}{%
%begin  現在までの研究状況＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	本研究では\textbf{フェイクニュースの早期自動検出}のために、記事と2件のコメントから\textbf{ユーザのコメントを1件生成・追加し真偽を分類}した。
	これは\textbf{コメントを集合知として分類に活用}できる一方、\textbf{早期検出には多数のコメントを望めない}ためである。
	実際にコメントを生成し真偽分類した場合が、生成しない場合\textbf{より多くのフェイクニュースを検出}した。
	...
	しかし、これらの手法において\textbf{ユーザの反応は拡散後でしか得られない}ため、早期の検出に活用できない。
	...
	本研究はニュースと寄せられたコメントを、
	本文1件に対し実際にSNS上で投稿されたコメントから(モデル構造の都合とデータ量確保のため)3件を対象に1ユニットとして扱った。
	...
	本研究では2点挙げられる。
	1点目は自らフェイクニュース拡散でみられる傾向から手法の検討に入ったことである。
	ユーザの反応は確かに真偽を判断する有力な情報に足る上に、SNS上の拡散スピードに対応することを念頭に考案した。
	2点目は直接真偽を入力せずに記事とコメントのみからコメントを生成するようモデル化して、
	真偽を評価する分類を補助させたことである。
	これによって、内容から予想されるコメントを生成する行為そのものが、
	真偽分類においてどのような影響を与えるか解明できる。
	...
	これは提案コメント生成モデルが、
	記事本文とコメントのみから信憑性による文章における傾向差異の学習に成功しており、コメントが少ない段階でもニュースとコメントにおける⽂章の傾向から真偽の判断が出来る可能性を示唆した。
