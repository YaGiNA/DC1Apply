...
%begin  現在までの研究状況＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	\textbf{予想されるユーザの反応をコメント生成DNNモデルで生成・置換}
	し分類して実現を目指した。
	...
	本研究は記事本文1件に加え、寄せられたコメントから(モデル構造の都合とデータ量確保のため)3件を1ユニットとした。
	%コメント生成と真偽分類はそれぞれモデルを独立させた。
	真偽分類では図\ref{fig:model}の通り、
	\textbf{生成による変化の調査}へ\textbf{1件生成・置換}し、未生成時と比較した。
	...
	\textbf{真偽分類に有効な情報を提供するか解明}を目指した点である。
