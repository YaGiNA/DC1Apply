...
%begin  研究の背景＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	%拡散を抑制する。
	今後はフェイクニュース早期発⾒モデルと新たに開発する判断根拠の⾔語化モデルを統合し、
	フェイクニュース拡散抑制システムを実現する。
	...
	%\noindent
	%●\textbf{問題点}
	前節の研究で実際にGroverを拡張し、コメントを生成することで多くのフェイクニュースを早期検出したため、今後はFake誤判定を防止する。
	...
	自然言語処理の発展により、違和感なき文章を生成可能となった。
	\textbf{フェイクニュースの拡散抑制には判断根拠の⾔語化が必要}のため、
	近年多く提案された説明付加DNNモデル\cite{shu2019defend}に注⽬した。
	%しかしながら、速報性を併せ持つ説明可能な検出モデルは今の所先行研究のいずれも行われていない。
	ただし前節で生成されたコメントは文法面で不可解な点が多く、生成コメントから判断の根拠とする説明の提供は難しい。
	

	これではいくらフェイクニュースを検出できても、
	\textbf{狼少年めいたモデルでその上判断の理由も説明できない状態}でユーザの信用を得るのは難しく、拡散の抑制にはならない。
	...
	ユーザからの信⽤を得るために、\textbf{フェイクニュースを誤りなく多く早期検出}への性能向上と、\textbf{説得力向上}のため判断根拠を提供する説明可能性を考慮すべきと着想に至った。
...
%begin  現在までの研究状況＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	フェイクニュースの拡散抑制へ、有識者が調査する\textbf{ファクトチェック}がある。
	しかし属人的な作業で、拡散されてから着手されるため、結果公表まで時間がかかる。
	ゆえに指摘が拡散されず、抑制に繋がらりにくい。
	そこで自動で検出するために、ニュース内容や添付メディア、
	ユーザの反応からディープニューラルネットワーク (DNN)による手法がある
	\cite{Wang:2018:EEA:3219819.3219903}。
	特に、\textbf{ユーザの反応は真偽によって大きな違いがある}(i.e. フェイクである指摘やbotの介入)ため、\textbf{集合知として活用}した研究もある\cite{Wu:2018:TFF:3159652.3159677}。
	...
	しかし\textbf{ユーザの反応は拡散後に得られる}ため、\textbf{これらの⼿法ではフェイクニュースを早期検出できない}問題がある。
