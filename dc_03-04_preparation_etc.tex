...
%begin  研究の背景＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
	...
	\textbf{急速な拡散を抑制}するためには、\textbf{早期発見}に加えて利用者に\textbf{判断根拠を言語化し警告}する必要がある。
	
	%\subsection{問題点}
	%\noindent
	%●\textbf{問題点}
	早期検出は、前節の研究でGroverを拡張しコメントを生成して実現したため、今後はFake誤判定を防止する。
	...
	言語化にモデルに関しては、自然言語処理の発展によって違和感なき文章を生成可能となった。
	...
	ただし前節提案モデルで生成されたコメントは文法面で不可解な点が多く、
